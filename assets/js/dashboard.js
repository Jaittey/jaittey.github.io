window.addEventListener('DOMContentLoaded',async()=>{
  try{
    ...
    Object.entries(values).forEach(([k,v])=>{const e=document.querySelector(`[data-stat="${k}"]`);if(e)e.textContent=['revenue','expense','profit'].includes(k)?SBUI.money(v):String(v)});
    const tbody=document.querySelector('#recentInvoices tbody');
    if(tbody){
      const rows=[...invoices].slice(-6).reverse();
      tbody.innerHTML=rows.length
        ? rows.map(x=>`<tr><td data-label="Invoice">${SBUI.escape(x.invoiceNumber||x.id)}</td><td data-label="Customer">${SBUI.escape(x.customerName||'Walk-in')}</td><td data-label="Total">${SBUI.money(x.total||x.amount)}</td><td data-label="Status"><span class="status ${String(x.status||'draft').toLowerCase()}">${SBUI.escape(x.status||'Draft')}</span></td></tr>`).join('')
        : '<tr><td colspan="4" class="empty">No invoices yet.</td></tr>';
    }
  }catch(e){SBUI.toast(e.message||'Could not load dashboard data.','error')}
});
