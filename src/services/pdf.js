...
export async function createBusinessPdf(kind, record, settings) {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`Name: ${record.customerName || '—'}`, 16, 76);
  doc.text(`Contact Number: ${record.contact || '—'}`, 16, 81);

  const addressLines = doc.splitTextToSize(
    `Customer Address: ${record.customerAddress || '—'}`,
    104,
  );
  doc.text(addressLines, 16, 86);

  doc.setFont('helvetica', 'bold');
  doc.text(isInvoice ? 'PAYMENT' : 'DATES', 194, 70, { align: 'right' });
  doc.setFont('helvetica', 'normal');

  if (isInvoice) {
    doc.text(record.paymentMethod || '—', 194, 76, { align: 'right' });
    doc.text(`Date: ${dateText(record.createdAt || new Date())}`, 194, 81, { align: 'right' });
  } else {
    doc.text(`Date: ${dateText(record.createdAt || new Date())}`, 194, 76, { align: 'right' });
    doc.text(`Expire Date: ${dateText(record.validUntil)}`, 194, 81, { align: 'right' });
  }

  const billToBottom = 86 + (addressLines.length * 5);
  autoTable(doc, {
    startY: Math.max(101, billToBottom + 6),
