  ...
  return <div className="app-layout v21-layout v219-layout v300-layout">
    ...
    <div className="main-area">
      <header className="topbar enterprise-topbar compact-mobile-topbar">
        ...
        <div className="topbar-title"><p className="eyebrow">SMALL BUSINESS (SB) v4.1 JS FAST</p><h1>{PAGE_TITLES[page]||'Workspace'}</h1><div className="workspace-switcher"><select value={activeBusinessId} onChange={(e)=>onBusinessChange?.(e.target.value)}>{businesses.map((item)=><option key={item.businessId} value={item.businessId}>{item.businessName||'Business'}</option>)}</select>{canRegisterBusiness&&<button type="button" onClick={onRegisterBusiness} title="Register your business">＋</button>}</div></div>
