...
import { configurationError } from '../config/firebase';
  ...
  return (
    <main className="login-page v21-login">
      ...
      <section className="login-card enterprise-login-card">
        ...
        <p className="eyebrow">DF7 BUSINESS v2.1.2 ENTERPRISE</p>
        ...
        <div className="login-mode-tabs">
          ...
          <button className={mode === 'register' ? 'active' : ''} onClick={() => setMode('register')}>Activate account</button>
        </div>
        ...
        {mode === 'google' ? (
          ...
          <form className="enterprise-login-form" onSubmit={submit}>
            ...
            <button className="button button-primary" disabled={loading}>
              {mode === 'register' ? 'Activate invited account' : 'Sign in'}
            </button>
          </form>
        )}
        ...
        <p className="login-security">Access is controlled by the administrator and Firestore permissions.</p>
      </section>
    </main>
  );
}
