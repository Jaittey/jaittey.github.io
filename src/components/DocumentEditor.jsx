...
export default function DocumentEditor({ open, type, initial, customers, products, settings, onClose, onSave }) {
  ...
  const empty = {
    customerId: '', customerName: '', contact: '', customerAddress: '', status: isInvoice ? 'BILLED' : 'DRAFT',
  ...
  const selectCustomer = (id) => {
    ...
    setForm((current) => ({ ...current, customerId: id, customerName: customer?.name || '', contact: customer?.phone || '', customerAddress: customer?.address || '' }));
  };
  ...
  return (
    <Modal open={open} title={initial?.id ? `Edit ${isInvoice ? 'invoice' : 'quotation'}` : `New ${isInvoice ? 'invoice' : 'quotation'}`} onClose={onClose}>
      <div className="form-grid">
        ...
        <label><span>Name</span><input value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} placeholder="Customer name" /></label>
        <label><span>Contact number</span><input inputMode="tel" value={form.contact} onChange={(e) => setForm({ ...form, contact: e.target.value })} placeholder="Contact number" /></label>
        <label className="form-span-2"><span>Customer address</span><textarea rows="3" value={form.customerAddress || ''} onChange={(e) => setForm({ ...form, customerAddress: e.target.value })} placeholder="Customer address" /></label>
