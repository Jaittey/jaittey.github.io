...
export default function Products({ products, settings, notify }) {
  ...
  const blank = { name: '', sku: '', quantity: 0, price: 0, threshold: 5 };
  const [form, setForm] = useState(blank);
  const open = (record = null) => { setEditing(record || {}); setForm(record || blank); };
  const filtered = products.filter((row) => `${row.name || ''} ${row.sku || ''}`.toLowerCase().includes(search.toLowerCase()));
  const save = async () => {
    if (!form.name.trim()) return;
    await saveRecord('products', { name: form.name.trim(), sku: String(form.sku || '').trim().toUpperCase(), quantity: safeNumber(form.quantity), price: safeNumber(form.price), threshold: safeNumber(form.threshold) }, editing?.id || null);
