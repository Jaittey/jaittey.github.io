...
export default function Quotes({ quotes, customers, products, settings, notify, markDriveConnected, openInvoices }) {
  ...
  const preview = async (record) => {
    try {
      ...
      notify(reason?.message || 'Could not create the quotation preview.', 'error');
    }
  };

  const download = async (record) => {
    try {
      downloadBlob(await makePdf(record), `${record.quoteNumber}.pdf`);
    } catch (reason) {
      notify(reason?.message || 'Could not create the quotation PDF.', 'error');
    }
  };
  ...
  const upload = async (record) => {
    try {
      ...
      const result = await uploadBusinessPdf(pdfBlob, `${record.quoteNumber}.pdf`, 'Quotations', settings.driveRootFolder);
  ...
  const convert = async (record) => {
    ...
    try {
      ...
      notify(reason?.message || 'Could not convert the quotation.', 'error');
    }
  };
  ...
  return <>
    ...
    <section className="panel">
      <div className="responsive-table">
        <table>
          <thead><tr><th>Quotation</th><th>Date</th><th>Customer</th><th>Status</th><th>Total</th><th>Actions</th></tr></thead>
          <tbody>
            {filtered.map((row) => <tr key={row.id}>
              ...
              <td>
                <div className="row-actions">
                  ...
                  <button onClick={() => convert(row)}>To invoice</button>
                  <button className="danger" onClick={async () => { if (confirm('Delete this quotation?')) { await deleteRecord('quotes', row.id); notify('Quotation deleted.'); } }}>Delete</button>
                </div>
              </td>
            </tr>)}
          </tbody>
