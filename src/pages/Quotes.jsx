  ...
  return <>
    ...
    <section className="panel">
      <div className="responsive-table"><table>
        <thead><tr><th className="select-column"><input type="checkbox" checked={allVisibleSelected} onChange={toggleAll} aria-label="Select all visible quotations" /></th><th>Quotation</th><th>Date</th><th>Customer</th><th>Status</th><th>Total</th><th>Actions</th></tr></thead>
        <tbody>{filtered.map((row) => <tr key={row.id} className={selected.includes(row.id) ? 'selected-row' : ''}>
          ...
          <td>{dateText(row.createdAt)}</td><td>{row.customerName}</td><td><span className={`status status-${row.status?.toLowerCase()}`}>{row.status}</span></td><td>{currency(row.total, settings.currency)}</td>
